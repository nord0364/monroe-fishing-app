...
import { WATER_DEPTHS, WATER_COLUMNS, STRUCTURE_TYPES } from '../../constants'
...
export default function DepthStructure({ fish, settings }: Props) {
  ...
  // Size by water depth
  const depthData = WATER_DEPTHS.map(depth => {
    const at = lmb.filter(f => f.waterDepth === depth)
    const avgW = at.length > 0
      ? +(at.reduce((s, f) => s + f.weightLbs + f.weightOz / 16, 0) / at.length).toFixed(2)
      : 0
    return { depth: depth.replace(' ft', ''), avg: avgW, count: at.length }
  }).filter(d => d.count > 0)
  ...
  return (
    <div className="space-y-5 pb-6">
      {/* Average weight by depth band */}
      {depthData.length > 0 ? (
        <div className="bg-slate-800 rounded-xl p-4 border border-slate-700">
          <h3 className="text-slate-200 font-semibold mb-1 text-sm">Avg Largemouth Weight by Depth</h3>
          <p className="text-slate-500 text-xs mb-3">Identifies where the bigger fish are holding</p>
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={depthData} layout="vertical">
              <CartesianGrid strokeDasharray="3 3" stroke="#334155" horizontal={false} />
              <XAxis type="number" unit="lb" tick={{ fontSize: 10, fill: '#94a3b8' }} />
              <YAxis dataKey="depth" type="category" tick={{ fontSize: 9, fill: '#94a3b8' }} width={70} />
              <Tooltip contentStyle={{ background: '#1e293b', border: '1px solid #334155', borderRadius: 8 }}
                labelStyle={{ color: '#94a3b8' }} itemStyle={{ color: '#e2e8f0' }} />
              <Bar dataKey="avg" fill="#0ea5e9" radius={[0, 4, 4, 0]} name="Avg weight" />
            </BarChart>
          </ResponsiveContainer>
          <div className="mt-2 space-y-1">
            {depthData.map(d => (
              <div key={d.depth} className="flex justify-between text-xs">
                <span className="text-slate-400">{d.depth}</span>
                <span className="text-slate-500">{d.count} catches · {d.avg}lb avg</span>
              </div>
            ))}
          </div>
        </div>
      ) : (
        <p className="text-slate-500 text-sm text-center py-4">Log catches with depth data to see analysis.</p>
      )}
